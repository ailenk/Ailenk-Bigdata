...
import ink.bignose.entity.BrandLike;
...
public class BrandLikeSink implements SinkFunction<BrandLike> {
    ...
    public void invoke(BrandLike value, Context context) throws Exception {
        ...
        Document doc = MongoUtils.findoneby("brandlikestatics","bignosePortrait",brand);
        if(doc == null){
            doc = new Document();
            doc.put("info",brand);
            doc.put("count",count);
        ...
        MongoUtils.saveorupdatemongo("brandlikestatics","bignosePortrait",doc);
    }
}
